...
from transnormer.swa_model import create_custom_causal_mask
...
def linear_attention(q, k, v, attn_mask):
    ...
    # print(f"energy shape: {energy.shape}, attn_mask shape: {attn_mask.shape}, v shape: {v.shape}, q shape: {q.shape}, k shape: {k.shape}")
...
class HeadWisePhybridAttention(nn.Module):
    def __init__(self, config: LlamaConfig,
        ...
        self.q_conv1d = nn.Conv1d(
            in_channels=self.tnorm_head_num * self.head_dim,
            out_channels=self.tnorm_head_num * self.head_dim,
            bias=self.use_conv_bias,
            kernel_size=self.conv_kernel_size,
            groups=self.tnorm_head_num * self.head_dim,
    ...
    def _init_conv_to_one(self):
        with torch.no_grad():
            ...
            for channel in range(self.tnorm_head_num * self.head_dim):
    ...
    def forward(
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            ...
            causal_mask = create_custom_causal_mask(attention_mask, key_states, first_tokens=10, last_tokens=128, head_policy=[0] * self.attn_head_num)
        ...
        # reshape
        q_tnorm = rearrange(q_tnorm, "b l (h d) -> b h l d", h=self.tnorm_head_num)
        k, v = map(
        ...
        if self.perm_tensor is not None:
            k = torch.index_select(k, dim=1, index=self.perm_tensor)
            v = torch.index_select(v, dim=1, index=self.perm_tensor)
        k = k[:, self.attn_head_num:, ...]
        v = v[:, self.attn_head_num:, ...]
        ...
        output_tnorm = rearrange(output_tnorm, "b n (h d) -> b h n d", h=self.tnorm_head_num)
        output = torch.cat([attn_output, output_tnorm], dim=1)
        if self.perm_tensor is not None:
            output = torch.index_select(output, dim=1, index=self.inverse_perm_tensor)
        ### MERGE
        # FIXME: inverse permute
