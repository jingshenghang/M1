...
class HeadWisePhybridAttention(nn.Module):
    def __init__(self, config: LlamaConfig,
        ...
        self.attn_head_num = self.num_heads
        ...
        if self.config.head_mask_perm_list:
            perm_lst = self.config.head_mask_perm_list[layer_idx]
            # Use register_buffer so these move automatically with .to(), .cuda(), etc.
            self.register_buffer("head_mask_perm_tensor", torch.tensor(perm_lst, dtype=torch.long))
            
        else:
            self.head_mask_perm_tensor = None
    ...
    def forward(
        ...
        # FIXME
        if False and past_key_value is not None:
        ...
        output = attn_output #torch.cat([attn_output, swa_output], dim=1)
        # if self.perm_tensor is not None:
        #     output = torch.index_select(output, dim=1, index=self.inverse_perm_tensor)

        if self.head_mask_perm_tensor is not None:
            output = torch.index_select(output, dim=1, index=self.head_mask_perm_tensor)
...
class TransnormerDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int, device, dtype):
        ...
        self.skip_attn = config.layer_mask_lst and not config.layer_mask_lst[layer_idx]  # whether to skip attention
