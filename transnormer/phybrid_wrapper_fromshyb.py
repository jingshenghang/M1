...
class TransnormerTransformerHybridModelWrapper(nn.Module, GenerationMixin):
    ...
    def __init__(self, checkpoint_path, transformer_model, transnormer_config, attn_layers, dtype, init_with_kqvo, load_from_hub=False, **kwargs):
        ...
        self.tower_num_layers = len(self.attn_layers)
        assert self.tower_num_layers == self.num_hidden_layers//2 #for phybrid need equal num of layers

        self.attn_ws_lst = nn.ParameterList([nn.Parameter(torch.tensor(1.0)) for _ in range(self.tower_num_layers)])
        self.transnorm_ws_lst = nn.ParameterList([nn.Parameter(torch.tensor(1.0)) for _ in range(self.tower_num_layers)])
        ...
        shybrid_checkpoint = '/work/ivan/data/Experiments/transnormer_distill_32B_1.5B_0.5_2k_conv-one/checkpoint-182667/pytorch_model.bin' #TODO - need to set manually
        weights = torch.load(shybrid_checkpoint, map_location="cpu")
        new_state_dict = self.state_dict()
        # Manually transfer only matching layers
        for key in weights.keys():
            if key in new_state_dict:
                new_state_dict[key] = weights[key]
        ...
        # self.decoder_attn_layers = [self.model.model.layers[i] for i in range(self.num_hidden_layers) if i in attn_layers]
        # self.decoder_attn_layers = nn.ModuleList(self.decoder_attn_layers)
        # self.decoder_transnormer_layers = [self.model.model.layers[i] for i in range(self.num_hidden_layers) if i not in attn_layers]
        # self.decoder_transnormer_layers = nn.ModuleList(self.decoder_transnormer_layers)
        
        

        #compute slopes for linear attention
        self._linear_attn_mask = torch.empty(0)
        self.slopes = self._build_slope_tensor(transnormer_config.num_attention_heads)
    ...
    def forward(
        ...
        slope_rates = [
            self.slopes.to(input_ids.device) for _ in range(self.tower_num_layers) #only for one tower
        ]
        
        # for idx, (decoder_attn_layer, decoder_transnormer_layer) in enumerate(zip(self.decoder_attn_layers, self.decoder_transnormer_layers)):
        for idx in range(self.tower_num_layers):
            attn_idx = 2*idx
            assert attn_idx in self.attn_layers
            decoder_attn_layer = self.model.model.layers[attn_idx]  
            transnorm_idx = 2*idx + 1
            assert transnorm_idx not in self.attn_layers
            decoder_transnormer_layer = self.model.model.layers[transnorm_idx]  

            if output_hidden_states:
                all_hidden_states += (hidden_states,)
            
            attn_layer_outputs = decoder_attn_layer(
                hidden_states,
            ...
            slope_rate = slope_rate * (1 - idx / (self.tower_num_layers - 1) + 1e-5)

            transnormer_layer_outputs = decoder_transnormer_layer(
                hidden_states,
            ...
            attn_w, transnormer_w = self.attn_ws_lst[idx], self.transnorm_ws_lst[idx]
            hidden_states = attn_w * attn_layer_outputs[0] + transnormer_w * transnormer_layer_outputs[0]

            if use_cache:  #TODO - check
                next_decoder_cache = transnormer_layer_outputs[2 if output_attentions else 1]
