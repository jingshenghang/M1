...
class SWAAttention(nn.Module):
    def __init__(self, config: LlamaConfig,
        ...
        if config.head_policy_dict is not None and config.head_policy_dict.get(str(layer_idx), None) is not None:
            self.head_policy = [1] * self.num_heads 
            for idx in config.head_policy_dict[str(layer_idx)]:
                self.head_policy[idx] = 0    
        else:
            self.head_policy = [0] * self.num_heads
    ...
    def forward(
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            ...
            # print(causal_mask)
            causal_mask = create_custom_causal_mask(causal_mask, key_states,
                                       first_tokens=10, last_tokens=128,
                                       head_policy=[0] * self.num_heads)
...
class TransnormerDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int, device, dtype):
        ...
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = SWAAttention(config=config, layer_idx=layer_idx)
