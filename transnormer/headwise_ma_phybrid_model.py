...
class HeadWisePhybridAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ### MAIN CHANGE OF HEADS SPECIALIZATION - AND IN THE END WHEN MERGE
        ...
        query_states = q[..., self.hidden_size // 2:]  # Take SECOND half of embedding to ATTN
        q_tnorm = q[..., :self.hidden_size // 2] # Take FIRST half to TRANSNORMER
        ...
        k = repeat_kv(k, self.num_key_value_groups//2)
        v = repeat_kv(v, self.num_key_value_groups//2)
        ...
        ### MERGE ##########################################################
        ### CHANGE ORDER FOR SPECIALIZATION MA
        # output = torch.cat([attn_output, output_tnorm], dim=-1)  # (b, n, h)
        output = torch.cat([output_tnorm, attn_output], dim=-1)  # (b, n, h)
