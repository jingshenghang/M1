...
class TransnormerTransformerHybridModelWrapper(nn.Module, GenerationMixin):
    ...
    def __init__(self, checkpoint_path, transformer_model, transnormer_config, attn_layers, dtype, init_with_kqvo, load_from_hub=False,  device=None, **kwargs): #
        ...
        device = f"cuda" if device is None else device
    ...
    @staticmethod
    def from_pretrained_local(pretrained_model_name, torch_dtype=torch.bfloat16,  attn_implementation="flash_attention_2", device=None): # ,
        ...
        return TransnormerTransformerHybridModelWrapper(pretrained_model_name, transformer_model, transnormer_config, transnormer_config.attn_layers, torch_dtype, init_with_kqvo=False, device=device ) #
    ...
    @staticmethod
    def from_pretrained(pretrained_model_name, torch_dtype=torch.bfloat16, attn_implementation="eager", device=None): #
        if os.path.exists(pretrained_model_name):
            return TransnormerTransformerHybridModelWrapper.from_pretrained_local(pretrained_model_name, torch_dtype, attn_implementation, device=device) #
        else:
