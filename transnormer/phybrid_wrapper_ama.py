...
class TransnormerTransformerHybridModelWrapper(nn.Module, GenerationMixin):
    ...
    def __init__(self, checkpoint_path, transformer_model, transnormer_config, attn_layers, dtype, init_with_kqvo, load_from_hub=False, **kwargs):
        ...
        self.tower_num_layers = len(self.attn_layers) - 2
        assert self.tower_num_layers == (self.num_hidden_layers-2)//2 #for phybrid need equal num of layers
    ...
    def forward(
        ...
        assert 0 in self.attn_layers
        decoder_attn_layer = self.model.model.layers[0] # first layer is always attention
        if output_hidden_states:
            all_hidden_states += (hidden_states,)
        layer_outputs = decoder_attn_layer(
                hidden_states,
        ...
        for idx in range(self.tower_num_layers):
            ...
            # print("index", idx, "transnorm", transnorm_idx, "attn", attn_idx)
              
            if output_hidden_states:
                all_hidden_states += (hidden_states,)

            #transnormer goes first 
        ...
        assert (self.num_hidden_layers-1) in self.attn_layers
        decoder_attn_layer = self.model.model.layers[-1] # last layer is always attention
        if output_hidden_states:
            all_hidden_states += (hidden_states,)
        layer_outputs = decoder_attn_layer(
                hidden_states,
